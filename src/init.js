...
export async function copyDir(srcDir, destDir, label, ext = ".md") {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      const sub = await copyDir(srcPath, destPath, null, ext);
      added += sub.added;
      updated += sub.updated;
...
export async function init(config) {
  ...
  try {
    ...
    for (const { src, dest, label, type, ext } of paths) {
      const srcDir = join(tmpDir, src);
      const destDir = join(cwd, dest);
      const { added, updated, unchanged, files } = await copyDir(srcDir, destDir, label, ext);
      totalAdded += added;
