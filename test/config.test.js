...
describe("buildPaths", () => {
  it("returns shared steering + skills + hooks when no types given", () => {
    ...
    expect(paths).toEqual([
      ...
      { src: "agents", dest: ".kiro/agents", label: "Custom Agents (shared)", type: "shared", ext: [".md"] },
    ]);
  });
  ...
  it("adds type-specific paths for each type", () => {
    ...
    expect(paths[7]).toEqual({
      src: "python/agents",
      dest: ".kiro/agents/python",
      label: "Custom Agents (python)",
      type: "python",
      ext: [".md"],
    });
  });
  ...
  it("handles multiple types", () => {
    ...
    expect(types).toEqual(["shared", "shared", "shared", "shared", "python", "python", "python", "python", "cdk", "cdk", "cdk", "cdk"]);
  });
